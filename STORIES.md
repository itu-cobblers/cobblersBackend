...
- **Backend:** partly built — `/submissions` and [`GET /api/submissions/{subId}`](CONTRACT.md#get-apisubmissionssubid-shared--full-detail-for-one-submission) exist; `AttendanceService.GetAttendanceAsync` exists but has **no controller route** yet; `SubmissionRecorded` is not built. The remaining reads are straightforward; the broadcast hooks into `SubmissionService`'s grading step.
