...
- **Backend:** REST half done — `/attendance`, `/submissions` and [`GET /api/submissions/{subId}`](CONTRACT.md#get-apisubmissionssubid-shared--full-detail-for-one-submission) are all built, each session-scoped read 404-ing on an unknown *or ended* room. **`SubmissionRecorded` is the only backend work left**: it hooks into `SubmissionService`'s grading step after `SaveChanges`, fires only when the submission has a `SessionId` (never solo) and never for `project`.
