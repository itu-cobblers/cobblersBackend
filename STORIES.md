...
## S10 — Teacher re-syncs full attendance + submissions on (re)connect
...
- **Transport:** REST (hydration) + SignalR (live deltas — `SubmissionRecorded` for grading; presence stays on the existing `ObserveSession` / `RosterUpdated`)
- **Contract:** ✅ decided — [`GET /api/sessions/{code}/attendance`](CONTRACT.md#get-apisessionscodeattendance-teacher-roster-hydration) (the ever-joined roll) + [`GET /api/sessions/{code}/submissions`](CONTRACT.md#get-apisessionscodesubmissions-teacher--all-thin-submissions-in-this-room) (every thin attempt in the room) + [`GET /api/submissions/{subId}`](CONTRACT.md#get-apisubmissionssubid-shared--full-detail-for-one-submission) (code + result for the row Col 4 has selected), [`SubmissionRecorded`](CONTRACT.md#live-progress-broadcasts-server--teacher-observers-in-the-room).
- **Frontend:** not built — on dashboard load, hydrate the roll from `/attendance` (every name gray) and the attempt list from `/submissions`, derive per-`(assignmentId, studentId)` status client-side, **then** start `ObserveSession` for the connected set (those names turn green) and subscribe to `RosterUpdated` + `SubmissionRecorded` for the live delta. Col 3 filters the hydrated attempt list to the selected assignment (and optionally student); Col 4's read-only code+result view (currently a `mockSubmissions` placeholder in `TeacherDashboard.tsx`) calls the shared detail endpoint for whichever `subId` is selected. Today the dashboard has only the live layer, so a reconnecting teacher sees just who's currently connected — anyone who stepped away has vanished.
- **Backend:** ✅ **done.** `/attendance`, `/submissions` and [`GET /api/submissions/{subId}`](CONTRACT.md#get-apisubmissionssubid-shared--full-detail-for-one-submission) are built, each session-scoped read 404-ing on an unknown *or ended* room; `SubmissionRecorded` fires from `SubmissionService.SubmitAsync` after `SaveChanges`, only when the submission has a `SessionId` (never solo) and never for `project`, carrying one `/submissions` row to the room's SignalR group. The broadcast is **best-effort** — a hub failure is logged, not surfaced, because the submission is already committed and failing the request would invite a duplicate resubmit.
...
- **Decisions already made:**
  ...
  - "passed" is per-(student, assignment) EXISTS a passing submission, **not** a row average; `project` assignments (`passed = null`) are excluded from pass lists. The backend ships raw attempts — **status is derived on the frontend**, there is no `/progress` endpoint. See [CONTRACT.md](CONTRACT.md#teacher-dashboard-hydration-attendance--submissions).
  - `/attendance` is the roll of everyone who has **ever** joined and is the denominator for `passedNum`/`totalNum` everywhere in the dashboard — Leave and disconnect are the same offline state, and neither shrinks it or deletes the `Attendance` row. **Presence (gray/green) is frontend-only**: green while that `studentId` is in the live roster, gray otherwise.
  - A `studentId` on the live roster but not yet on the hydrated roll (first join while the teacher watches) is **appended** client-side — no separate attendance event. See [Roster events](CONTRACT.md#roster-events-server--teacher-observers-in-the-room).
  - Both submission-history `GET` list endpoints (this one, and S5's student-facing one) stay **thin** — `subId` + outcome, no `content`/`result`, and no denormalized `displayName` (join against `/attendance`). Either side gets the actual code + result the same way: one shared [`GET /api/submissions/{subId}`](CONTRACT.md#get-apisubmissionssubid-shared--full-detail-for-one-submission), fetched only for the one row currently selected. No teacher-only "fat" list endpoint.
  - `/submissions` is deliberately **unscoped and unfiltered** — one flat list per hydrate, sorted newest-first only. The dashboard needs both the by-assignment and by-student cuts depending on selection, so it groups client-side rather than re-fetching per selection.
  - `SubmissionRecorded` is a point-patch delta (the thin attempt row itself, same shape as one `/submissions` row), not a full-list re-broadcast — the dashboard prepends it and re-derives the affected cell rather than re-fetching.
- **Done when:** a teacher who reloads mid-class sees the full roll of everyone who joined — gray until each reconnects — **and** each student's passed assignments, not just who's currently connected; and Col 1's pass-rate badges / Col 2's status dots update live off `RosterUpdated` + `SubmissionRecorded` without re-polling.
...
- ~~Teacher sees live student progress via a `SubmissionRecorded` broadcast~~ — contract decided, see [S10](#s10--teacher-re-syncs-full-attendance--submissions-on-reconnect) and [CONTRACT.md](CONTRACT.md#live-progress-broadcasts-server--teacher-observers-in-the-room); not yet implemented.
